...
const RitualV: NextPage = () => {
  ...
  return (
    <div className="w-full h-screen bg-[#F5F5F5] flex flex-col items-center justify-between py-4 px-4">
      {/* Top section with description */}
      <div className="w-full max-w-4xl">
        <div 
          ref={descriptionRef} 
          className="text-center cursor-pointer px-4"
          style={{ fontSize: 'clamp(16px, 4vw, 32px)', lineHeight: '1.3' }}
          onMouseEnter={() => handleHover(setDescriptionText, descriptionText)}
        />
      </div>

      {/* Center section with logo and text */}
      <div className="flex items-center gap-2" style={{ fontSize: 'clamp(32px, 6vw, 56px)' }}>
        <svg 
          viewBox="0 0 100 100" 
      ...
      <div className="flex flex-col items-center gap-2 md:gap-4">
        {/* Countdown container */}
        <div className="border border-black p-3 md:p-8 w-full max-w-[500px]">
          <div className="flex justify-between items-end px-2 md:px-4">
            <div className="flex flex-col items-center">
              <span ref={daysRef} style={{ fontSize: 'clamp(20px, 5vw, 48px)', fontWeight: '400' }}>
                {days}
              </span>
              <span className="text-[10px] md:text-sm text-gray-600">days</span>
            </div>
            <div className="flex flex-col items-center">
              <span ref={hoursRef} style={{ fontSize: 'clamp(20px, 5vw, 48px)', fontWeight: '400' }}>
                {hours}
              </span>
              <span className="text-[10px] md:text-sm text-gray-600">hours</span>
            </div>
            <div className="flex flex-col items-center">
              <span ref={minutesRef} style={{ fontSize: 'clamp(20px, 5vw, 48px)', fontWeight: '400' }}>
                {minutes}
              </span>
              <span className="text-[10px] md:text-sm text-gray-600">mins</span>
            </div>
            <div className="flex flex-col items-center">
              <span ref={secondsRef} style={{ fontSize: 'clamp(20px, 5vw, 48px)', fontWeight: '400' }}>
                {seconds}
              </span>
              <span className="text-[10px] md:text-sm text-gray-600">seconds</span>
            </div>
          </div>
        </div>
